...
function isCryptoEngine(engine: unknown): engine is ICryptoEngine {
	return engine && typeof engine === "object" && "crypto" in engine
		? true
		: false;
}
	...
	if (
		...
		if (typeof (global as any)[process.pid] === "undefined") {
			...
			if (typeof (global as any)[process.pid] !== "object") {
				throw new Error(
					`Name global.${process.pid} already exists and it is not an object`
				);
			}
		}
		...
		if (typeof (global as any)[process.pid].pkijs === "undefined") {
			...
			if (typeof (global as any)[process.pid].pkijs !== "object") {
				throw new Error(
					`Name global.${process.pid}.pkijs already exists and it is not an object`
				);
			}
		}
 ...
 */
export function getOIDByAlgorithm(
	algorithm: Algorithm,
	safety?: boolean,
	target?: string
) {
 ...
 */
export function createCMSECDSASignature(
...
export function createECDSASignatureFromCMS(
	...
	const rValueView =
		cmsSignature.valueBlock.value[0].convertFromDER().valueBlock.valueHexView;
	const sValueView =
		cmsSignature.valueBlock.value[1].convertFromDER().valueBlock.valueHexView;
...
export function getAlgorithmByOID(
	oid: string,
	safety = false,
	target?: string
): any {
...
async function kdfWithCounter(
	...
	if (Counter > 255)
		throw new ArgumentError(
			"Please set 'Counter' argument to value less or equal to 255"
		);
	//#endregion
...
export async function kdf(
	...
	for (let i = 1; i <= maxCounter; i++)
		incomingResult.push(
			await kdfWithCounter(hashFunction, Zbuffer, i, SharedInfo, crypto)
		);
	//#endregion
